...
double PoseFuser::fusePose(Scan2D* curScan, const Pose2D& estPose, const Pose2D& odoMotion, const Pose2D& lastPose,
                           Pose2D& fusedPose, Eigen::Matrix3d& fusedCov) {
  // ICPの共分散
  dass->findCorrespondence(curScan, estPose);  // 推定位置estPoseで現在スキャン点群と参照スキャン点群の対応付け
  double ratio = cvc.calIcpCovariance(estPose, dass->curLps, dass->refLps,
                                      ecov);  // ここで得られるのは、地図座標系での位置の共分散

  // オドメトリの位置と共分散。速度運動モデルを使うと、短時間では共分散が小さすぎるため、簡易版で大きめに計算する
  ...
  CovarianceCalculator::rotateCovariance(estPose, mcovL,
                                         mcov);  // 現在位置estPoseで回転させて、地図座標系での共分散mcovを得る

  // evoc, mcov, covともに、lastPoseを原点とした局所座標系での値
...
void PoseFuser::calOdometryCovariance(const Pose2D& odoMotion, const Pose2D& lastPose, Eigen::Matrix3d& mcov) {
  ...
  double dT = 0.1;
  cvc.calMotionCovarianceSimple(odoMotion, dT, mcovL);  // オドメトリで得た移動量の共分散（簡易版）
  CovarianceCalculator::rotateCovariance(lastPose, mcovL,
                                         mcov);  // 直前位置lastPoseで回転させて、位置の共分散mcovを得る
}

// 2つの正規分布を混合する。muは平均、cvは共分散
double PoseFuser::fuse(const Eigen::Vector3d& mu1, const Eigen::Matrix3d& cv1, const Eigen::Vector3d& mu2,
  ...
  // 角度の補正。融合時に連続性を保つため
  Eigen::Vector3d mu11 = mu1;  // ICPの方向をオドメトリに合わせる
